h2. Bezpieczeństwo w Ruby On Rails
...
* Wszystkie _(highlight)wyróżnione sposoby obrony_
...
"artykułem na ten temat w Wikipedii":http//en.wikipedia.org/wiki/Md5#Vulnerability -
przyp. red.).
 
h4. Przechwytywanie sesji
...
- Wywęszenie cookies w niezabezpieczonej sieci. Bezprzewodowa sieć może posłużyć jako przykład takiej sieci. W nieszyfrowanej sieci bezprzewodowej śledzenie ruchu wszystkich podłączonych klientów jest szczególnie łatwe. Jest to jeden z powodów, dla których powinniśy zrezygnować z pracy w kawiarniach. Dla projektantów aplikacji internetowych oznacza to _(highlight)konieczność zapewnienia bezpiecznego połączenia przez SSL_.
 
- Większość ludzi nie kasuje cookies po pracy na publicznym sprzęcie. Więc jeśli ostatni użytkownik nie wylogował się z aplikacji internetowej, ktoś inny może się pod niego podszywać. Dlatego ważne jest, aby zapewnić użytkownikowi _(highlight)widoczny przycisk do wylogowania_.
...
Głównym celem większości atakujących jest korzyść finansowa. Ceny za skradzione loginy do kont bankowych na czarnym rynku są wyceniane od 10 do 1000 dolarów (w zależności od środków na koncie). Numery kart kredytowych są warte od 0,40 do 20 dolarów. Za konta na stronach z aukcjami online – 1 do 8 dolarów. Hasła do kont mailowych kosztują od 4 do 30 dolarów. Dane pochodzą ze strony: "Symantec Global Internet Security Threat Report":http//eval.symantec.com/mktginfo/enterprise/white_papers/b-whitepaper_internet_security_threat_report_xiii_04-2008.en-us.pdf. 
 
h4. Wskazówki dotyczące sesji
...
- _(highlight)Ważne dane nie powinny być przechowywane w sesjach_. Gdy
...
- Rozmiar cookies nie może przekraczać 4K. Nie stanowi to większego problemu,
  ...
  się do tego co napisałem wcześniej. _(highlight)Przechowywanie np.  bazodanowego identyfikatora zalogowanego użytkownika jest zazwyczaj ok_.
...
- Użytkownik może zobaczyć wszystko co przechowujesz w sesji, ponieważ
  ...
  Base64, ale nie szyfrowane). Więc zapewne _(highlight)nie chciałbyś przechowywać tu żadnych sekretnych danych_. Żeby zapobiec manipulacji danymi sesji, wyliczany jest skrót na podstawie danych sesji oraz tajnego klucza po stronie serwera i umieszczany na końcu pliku cookie.
...
nieskompromitowany, SHA512). Więc _(highlight)nie używaj prostych kluczy takich jak słowa ze słownika oraz kluczy poniżej 30 znaków_. Umieść swój tajny klucz w environment.rb: 
...
h4. Ataki typu Replay na sesje CookieStore.
...
Najlepszym _(highlight)rozwiązaniem jest przechowywanie tego rodzaju danych nie w sesji, lecz w bazie danych_. W tym przypadku kredyt powinien być przechowywany w bazie danych, a identyfikator zalogowanego użytkownika w sesji.

h4. Atak typu Session fixation
...
# Teraz atakujący może zmusić przeglądarkę użytkownika do korzystania z jego
  ...
  cookie innej domeny (z powodu zasady tożsamego pochodzenia - ang. <em>same origin policy</em>), atakujący musi uruchomić JavaScript z domeny docelowej aplikacji
  internetowej. Wstrzyknięcie (ang. <em>injection</em>) własnego kodu JavaScript do aplikacji będącej obiektem ataku jest możliwe dzięki metodom takim jak XSS. Oto przykład: 
...
h4. Sposoby obrony przed atakiem typu session fixation
 
-- _Jedna linia kodu może ochronić Cię przed atakiem typu session fixation._
  
Najskuteczniejszą ochroną jest _(highlight)wydanie nowego identyfikatora sesji_
...
+SessionsController#create+. Zwróć uwagę, że ta akcja usuwa wszystkie wartości z sesji, _(highlight) więc musisz przenieść je do nowej sesji_.
  
Innym sposobem obrony jest _(highlight)zapisywanie specyficznych właściwości użytkownika w sesji,_ weryfikowanie ich przy każdym żądaniu i odmowa dostępu, jeśli informacje te nie pasują do siebie. Takimi właściwościami mogą być adres IP lub nazwa przeglądarki internetowej, choć ta ostatnia nie jest aż tak bardzo charakterystyczna dla użytkownika. Podczas zapisywania adresu IP, musisz pamiętać, że istnieją dostawcy usług internetowych oraz duże organizacje, które oferują swoim użytkownikom dostęp do Internetu przez proxy. _(highlight) Adresy IP tych użytkowników mogą ulegać zmianom w trakcie sesji,_ więc nie będą oni mogli korzystać z aplikacji, lub tylko w ograniczonym zakresie.
...
bezpieczniejsze. Oto przykład, jak _(highlight)ustawić wygasanie sesji w tabeli bazy danych_. +Session.sweep("20m")+ powoduje wygaśnięcie sesji, które były
...
dotyczących bezpieczeństwa - _(highlight)CSRF jest istotną kwestią bezpieczeństwa_.

h4. Sposoby ochrony przed atakiem typu CSRF 
...
* Interakcja przypomina _(highlight)zapytanie_ (np. jeśli jest to bezpieczna operacja taka jak kwerenda, operacja czytania, albo wyszukiwania.
 
*Używaj  POST jeśli:*
 
* Interakcja przypomina _(highlight)komendę_ lub
* Interakcja _(highlight)powoduje zmianę_ zasobów w sposób zauważalny dla użytkownika (np. subskrypcja serwisu) lub
* Użytkownik _(highlight)jest odpowiedzialny za wynik działania_ interakcji.
...
_(highlight)Metody +verify+ w kontrolerze sprawdza czy konkretne akcje mogą być wykonywane za pomocą określonego typu żądania_. Oto przykład, w którym sprawdzane jest wykonanie akcji +transfer+ przez żądanie typu POST. Jeżeli akcja zostanie wywołana za pomocą jakiegokolwiek innego typu żądania HTTP, zostanie ona przekierowana
do akcji +list+.
...
Ale to dopiero pierwszy krok, ponieważ _(highlight)żądanie POST również może być wysłane automatycznie_. Oto przykład linku, który wyświetli
...
ofiary.  _(highlight)Rozwiązaniem tego problemu może być użycie tokena zabezpieczającego w żądaniach typu non-GET_ sprawdzanego po stronie serwera. W
...
_(highlight)przekazywaniu tylko oczekiwanych parametrów w akcji +legacy+_ (z
wykorzystaniem tzw. białej listy zamiast usuwania nieoczekiwanych parametrów). 
_(highlight)Dlatego każde akcję pozwalającą na przekierowanie do adresu URL, należy sprawdzać za pomocą białej listy lub z wykorzystaniem wyrażeń regularnych_. 
 
h5. Niezależny atak typu XSS
...
_(highlight)nie powinieneś pozwalać użytkownikowi na określanie adresu URL (lub jego części) w przekierowaniach_.
 
h4. Wysyłanie (upload) plików
...
plików. _(highlight)Nazwy plików, które wybierze użytkownik zawsze powinny być filtrowane_ ponieważ atakujący może użyć złośliwych nazw plików żeby nadpisać
...
Gdy filtrujesz pliki nadesłane przez użytkownika, <em class="highlight">nie staraj się usunąć złośliwych części</em>. Pomyśl o sytuacji, gdy aplikacja internetowa usuwa
wszystkie +../+ w nazwie pliku, a atakujący użyje takiej nazwy: +....//+ –
rezultat będzie taki: +../+. Najlepszym rozwiązaniem jest podejście zwane
białą listą, dzięki której <em class="highlight"> można sprawdzić czy nazwa pliu zawiera wyłącznie dozwolone znaki</em>. Jest to alternatywa dla podejścia tzw.
...
"pluginie attachment_fu":http//github.com/technoweenie/attachment_fu/tree/master:
...
jest jego _(highlight)podatność na ataki typu danial-of-service_. Atakujący
...
Najlepszym rozwiązaniem jest _(highlight)asynchroniczne przetwarzanie plików (w szczególności obrazków i filmów)_: zapisz plik i ustaw harmonogram
przetwarzania żądania w bazie danych. Niezależny proces zajmie się przetwarzaniem plików w tle. 
 
h4. Kod wykonywalny we wczytywanych plikach
...
_(highlight)Jeśli +DocumentRoot+ w Apache odnosi się do katalogu +/public+ Twojej aplikacji, nie umieszczaj w nim wczytywanych plików,_ 
...
_(highlight)sprawdzenie czy żądany plik znajduje się w tym katalogu, w którym się spodziewasz_:
...
h3. Ochrona intranetu i panelu administratora
...
"Trojan":http//www.symantec.com/enterprise/security_response/weblog/2007/08/a_monster_trojan.html,
...
Zapoznaj się z sekcją o atakach i ochronie przed XSS. _(highlight)Zalecane jest korzystanie z wtyczki SafeErb_ również w intranecie lub interfejsie administratora. 
...
pomocą "CSRF":http//www.0x000000.com/index.php?i=213&bin=11010101. Jeżeli
...
Informacje o _(highlight)ochronie przeciwko CSRF w interfejsach administracyjnych i aplikacjach intranetowych, znajdują się w dziale o ochronie przeciw CSRF_. 
  
h4. Dodatkowe środki ostrożności 
...
* Bardzo ważne jest, _(highlight)żeby myśleć o najgorszym przypadku_: Co
  zrobić, jeśli ktoś naprawdę przejmie moje pliki cookie lub dane
  uwierzytelniające użytkownika. Można _(highlight)wprowadzić role_ w
  interfejsie administratora, aby ograniczyć możliwości atakującego. Można też
  wprowadzić _(highlight)odrębny login i hasło_ do interfejsu administratora,
  inne niż te używane do publicznej części aplikacji albo _(highlight)specjalne hasło do bardzo poważnych działań_?
* Czy admin naprawdę ma dostęp do interfejsu z każdego miejsca na świecie?
  Pomyśl o _(highlight)ograniczeniu logowania do kilka adresów IP_. Zbadaj
...
* _(highlight)Umieść specjalny interfejs administracyjny w sub-domenie_ takiej
...
_(highlight)metoda +attr_accessible+_. Jest to dokładne przeciwieństwo
+attr_protected+, ponieważ _(highlight)dotyczy listy atrybutów, które będą dostępne_. Wszystkie inne atrybuty będą chronione. W ten sposób nigdy nie
...
Istnieje kilka wtyczek obsługujących uwierzytelnianie i autoryzację, dostępnych w Railsach. Te lepsze zapisują tylko zaszyfrowane hasła, a nie te czysto tekstowe. Najbardziej popularną wtyczką jest _(highlight)restful_authentication,_ która chroni również przed atakami typu session fixation. Jednak wcześniejsze wersje w niektórych okolicznościach dopuszczały do logowanie bez nazwy użytkownika i hasła.
...
jego zalogowanie. Więcej o tym problemie możesz przeczytać na "moim blogu":http://www.rorsecurity.info/2007/10/28/restful_authentication-login-security/. _(highlight)Od czasu do czasu powinieneś aktualizować swoje wtyczki_. Ponadto możesz przeglądnąć swoją aplikację, żeby znaleźć więcej takich wad.

h4. Ataki typu brute-force na konta użytkowników
...
W celu złagodzenia takich ataków, _(highlight)wyświetlaj ogólne komunikat o błędzie, również na stronach z zapomnianymi hasłami_. Ponadto, możesz
_(highlight)wymagać, aby wprowadzić CAPTCHA po kilku błędach logowania z określonego adresu IP_. Należy jednak pamiętać, że nie jest to stuprocentowa
...
h4. Przejęcie konta użytkownika
...
IMG-tag, który odpala CSRF. Można temu zaradzić i _(highlight)stworzyć formularz zmiany hasła odporny na CSRF_ i _(highlight)wymagać od użytkownika, aby wprowadzał stare hasło przy jego zmianie_.
...
można również _(highlight)wymagać od użytkownika, aby wprowadził hasło przy zmianie adresu e-mail_. 
...
problematyczne. Popularnym CAPTCHA API jest "reCAPTCHA":http//recaptcha.net/,
...
zostały złamane. Ponadto używanie reCAPTCHA pomaga w tworzeniu cyfrowych wersji starych książek. "ReCAPTCHA":http//ambethia.com/recaptcha/ jest także Railsową wtyczką, o takiej samej nazwie jak API.
...
Bardziej wyrafinowane negatywne CAPTCHA możesz znaleźć na "blogu Batchelder Ned’a":http//nedbatchelder.com/text/stopbots.html: 
...
_(highlight)filtrować niektóre parametry żądań z logu_ dzięki metodzie
...
Bruce Schneier, technolog zabezpieczeń, "przeanalizował":http//www.schneier.com/blog/archives/2006/12/realworld_passw.html 34000 występujących w świecie rzeczywistym nazw użytkowników i haseł ze wspomnianej wcześniej próby wyłudzenia danych z MySpace. Okazuje się, że większość haseł jest dość łatwa do złamania. Oto 20 najpopularniejszych haseł: 
...
zapisać tylko _(highlight)pierwsze litery zdania, która można łatwo zapamiętać_. Na przykład "The quick brown fox jumps over the lazy dog" będzie
...
łańcuchu. Jednakże _(highlight)w Ruby ^ i $ odpowiada początkowi i końcu *linii*_. A więc taka nazwa pliku przechodzi przez filtr bez problemów: 
...
_(highlight)sprawdzaj również prawa dostępu użytkownika_: 
...
_(highlight)żadne dane wprowadzane przez użytkownika nie są bezpieczne, dopóki nie zostanie udowodnione, że jest inaczej. Użytkownik może manipulować każdym parametrem_. 
...
zmienić każde ukryte pole formularza. _(highlight)JavaScript może być wykorzystywany w celu weryfikacji danych użytkownika, ale z pewnością nie po to, by zapobiec wysyłaniu złośliwych żądań z nieoczekiwanymi wartościami_.
...
h4. Białe listy vs Czarne listy
...
filtr antyspamowy), _(highlight)lepiej jest używać właśnie jej_: 
...
Również drugie zapytanie zmienia nazwy niektórych kolumn korzystając z +AS+ tak aby aplikacja internetowa wyświetlała wartości z tabeli użytkowników. Pamiętaj, aby zaktualizować Railsy "przynajmniej do wersji 2.1.1":http//www.rorsecurity.info/2008/09/08/sql-injection-issue-in-limit-and-offset-parameter/. 
...
_(highlight)Korzystając z +Model.find(id)+ lub +Model.find_by_something(something)+ automatycznie stosujesz ochronę_. Ale fragmenty SQL, szczególnie _(highlight)w warunkach (+:conditions => "..."+), metody +connection.execute()+ lub +Model.find_by_sql()+ muszą być zabezpieczone ręcznie_. 
...
_(highlight)Nabierz nawyku myślenia o konsekwencjach bezpieczeństwa gdy korzystasz z zewnętrznego ciągu znaków w zapytaniach SQL_. 

h4. Atak Cross-Site Scripting (XSS)
...
"MPack":http//pandalabs.pandasecurity.com/archive/MPack-uncovered_2100_.aspx
...
"zostało zhakowanych":http//www.0x000000.com/?i=556 w ten sposób. 
...
h5. Ataki HTML/JavaScript Injection
...
połączeniu z HTML. _(highlight)Czyszczenie danych wprowadzanych przez użytkownika jest niezbędne_. 
...
"httpOnly":http//dev.rubyonrails.org/ticket/8895 cookies, żeby
+document.cookie+ nie mógł być odczytany przez JavaScript. Cookies +httpOnly+ mogą być używane pod IE v6.SP1, Firefoxoxem v2.0.0.5 i Operą 9.5. Safari nadal ignoruje tą opcję. Ale w innych, starszych przeglądarkach (np. WebTV i IE 5.5 na Macu) może to powodować problemy z ładowaniem się stron. Ostrzegamy, że pliki cookie "nadal będą widoczne przy użyciu technologii Ajax":http//ha.ckers.org/blog/20070719/firefox-implements-httponly-and-is-vulnerable-to-xmlhttprequest/.
 
h6. Atak typu Defacement
...
ataku] prawdziwych włoskich stron przy użyciu "frameworku ataku MPack":http//isc.sans.org/diary.html?storyid=3015. MPack próbuje zainstalować złośliwe oprogramowanie poprzez luki w przeglądarce internetowej - jest bardzo skuteczny, 50% ataków kończy się sukcesem.
...
_(highlight)Bardzo ważne jest filtrowanie danych wejściowych, ale równie ważne jest jego cytowanie (ang. escaping) w aplikacjach internetowych_.
 
W szczególności dla XSS, ważne jest, aby stosować _(highlight)białą listę filtrowania zamiast czarnej_. Biała lista określa dozwolone wartości w przeciwieństwie do wartości niedozwolonych. Czarne listy nigdy nie są kompletne.
...
W drugim etapie, _(highlight)dobrą praktyką jest cytowanie wszystkich danych wyjściowych aplikacji,_ zwłaszcza gdy ponownie wyświetlane są dane wprowadzone
przez użytkownika, które nie zostały przefiltrowane (jak we wcześniej
wspomnianym przykładzie formularza wyszukiwania). _(highlight)Używaj metody +escapeHTML()+ (lub jej aliasu +h()+)_ żeby zastąpić znaki wejścia HTML
&,",<,> przez ich nieinterpretowane reprezentacje w formacie HTML (+&amp;amp;+,
+&amp;quot;+, +&amp;lt+; oraz +&amp;gt;+). Jednak może się zdarzyć, że
programista zapomni ich użyć, więc _(highlight)zaleca się korzystanie z "wtyczki SafeErb":http//safe-erb.rubyforge.org/svn/plugins/safe_erb/_. SafeErb
przypomina o cytowaniu ciągów znaków pochodzących z zewnętrznych źródeł.
  
h6. Zaciemnianie (ang. obfuscation) i ataki z wykorzystaniem kodowania znaków
...
Poniżej znajduje się wyciąg z "robaka":http//groovin.net/stuff/yammer.txt
"Js.Yamanner@m":http//www.symantec.com/security_response/writeup.jsp?docid=2006-061211-4111-99&tabid=1 Yahoo! Mail. 
...
atakujący cztery włoskie usługi webmailowe. Więcej szczegółów oraz demonstracja wideo znajdują się na stronie "Rosario Valotta":http//rosario.valotta.googlepages.com/home. Oba webmailowe robaki mają na celu zbieranie adresów e-mail, czyli coś, za co haker może zarobić duże pieniądze. 
...
Wartość "moz-binding":http//www.securiteam.com/securitynews/5LP051FHPE.html CSS okazała się być innym sposobem na wprowadzanie JavaScript do CSS w przeglądarkach opartych na Gecko (na przykład Firefox). 
...
funkcją, jednak nie obawiam się stosować białych list do filtrowania CSS. _(highlight)Jeśli chcesz zezwolić na niestandardowe kolory lub obrazek, możesz pozwolić użytkownikowi je wybrać i tworzyć CSS w aplikacji internetowej_. Użyj Railsowej metody +sanitize()+ jako modelu dla białej listy filtrowania CSS, jeśli naprawdę jej potrzebujesz. 
 
h4. Atak typu Textile Injection
  
-- _Jeśli chcesz zapewnić formatowanie tekstu inne niż HTML (ze względu na bezpieczeństwo), powinieneś użyć języka mark-up, który jest przekształcany na HTML po stronie serwera. "RedCloth":http//whytheluckystiff.net/ruby/redcloth/ jest takim język dla Ruby, ale bez stosowania środków ostrożności, jest również podatny na XSS._ 
...
"nowszą wersje 4":http//www.redcloth.org, w której usunięto poważne
błędy. Jednak nawet ta wersja posiada "kilka błędów bezpieczeństwa":http//www.rorsecurity.info/journal/2008/10/13/new-redcloth-security.html, więc sposoby ochrony nadal obowiązują. Oto przykład dla wersji 3.0.4: 
...
Zaleca się _(highlight)korzystanie z  RedCloth w połączeniu z białą listą do filtrowania wejścia,_ jak zostało to opisane w sposobach ochrony przeciwko XSS.
 
h4. Atak typu Ajax Injection
...
widoku, _(highlight)musisz cytować wartości zwracane w akcji_. W przeciwnym
...
widokiem lub częścię widok po stronie serwera. _(highlight)Jeśli zezwalasz na wyświetlanie danych wprowadzonych przez użytkownika w szablonach RJS, użyj cytowania korzystając z +escape_javascript()+ w funkcjach JavaScript i +h()+ w HTML_. 
W przeciwnym wypadku atakujący może wykonać arbitralny kod JavaScript. 
 
h4. Atak typu Command Line Injection
...
Jako ochrony _(highlight)użyj metody +system(command, params)+, która bezpiecznie przekazuje parametry linii poleceń_. 
...
większym lub mniejszym wysiłkiem. _(highlight)Należy pamiętać, aby unikać wyświetlania tych pól nagłówków_ na przykład, kiedy wyświetlasz agenta użytkownika w sferze administracyjnej.
 
Poza tym, _(highlight)warto wiedzieć, co robisz przy budowie nagłówków odpowiedzi, częściowo opartych na wejściu użytkownika_. Na przykład jeśli
...
_(highlight)Sposoby ataku typu header injection są oparte na wstawianiu znaków CRLF do pola nagłówka_. A co atakujący może zrobić z fałszywymi
...
_(highlight)Railsy 2.1.2 unikają wstawiania tych znaków w polu +Location+ w metodzie +redirect_to+. Upewnij się, że zrobisz to sam podczas budowania innych pól nagłówka na podstawie danych wprowadzonych przez użytkownika_. 
 
h5. Rozdzielanie odpowiedzi
...
miejsce tylko i wyłącznie w przypadku połączeń typu Keep-Alive (wiele przeglądarek używa jednorazowych połączeń). Ale nie można na tym zbytnio polegać. _(highlight)W każdym razie jest to poważny błąd i należy zaktualizować Railsy do wersji 2.0.5 lub 2.1.2 w celu wyeliminowania ryzyka wystąpienia header injection (i tym samym rozdzielania odpowiedzi)_. 
...
- Zapisz się do "listy mailingowej":http//groups.google.com/group/rubyonrails-security bezpieczeństwa w Railsach
- "Bądź na bieżąco z pozostałymi warstwami aplikacji":http//secunia.com/
  (również posiadają cotygodniowy biuletyn)
- "Dobry blog o bezpieczeństwie":http//ha.ckers.org/blog/ zawierający "ściągę z Cross-Site scripting":http//ha.ckers.org/xss.html
- Inny "dobry blog o bezpieczeństwie":http//www.0x000000.com/ również zawierający ściągawki.
