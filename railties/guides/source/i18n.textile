...
To oznacza, że w ustawieniach regionalnych +:en+ klucz _hello_ zostanie zamieniony na łańcuch znaków _Hello world_. Każdy string wewnątrz Railsów może być internacjonalizowany w ten sposób, zobacz na przykład komunikaty walidacji modułu Active Record w pliku "+activerecord/lib/active_record/locale/en.yml+":http://github.com/rails/rails/blob/master/activerecord/lib/active_record/locale/en.yml albo formaty czasu i daty w pliku "+activesupport/lib/active_support/locale/en.yml+":http://github.com/rails/rails/blob/master/activesupport/lib/active_support/locale/en.yml. Możesz użyć formatu YAML bądź też standardowych tablic asocjacyjnych Ruby, aby przechowywać tłumaczenia w domyślnym (prostym) stanie końcowym procesu.
...
NOTE: Railsy dodają do widoków helpera +t+ (od ang. _translate_ = _tłumaczenie_). Wychwyci on brakujące tłumaczenia i wymieni komunikat błędu na +&lt;span class="translation_missing"&gt;+.
...
Dla przykładu, zamiast standardowego komunikatu błędu +"can not be blank"+ (ang. _nie może być pusty_), możesz użyć nazwy atrybutu: +"Please fill in your {{attribute}}"+ (ang._ Proszę, podaj wartość atrybutu {{attribute}}_).
...
Jeśli zauważysz, że w naszym "repozytorium przykładowych tłumaczeń":http://github.com/svenfuchs/rails-i18n/tree/master/rails/locale brakuje Twojego języka, prosimy, "_otwórz_)":http://github.com/guides/fork-a-project-and-submit-your-modifications repozytorium, dopisz dane i "wyślij je nam":http://github.com/guides/pull-requests.
